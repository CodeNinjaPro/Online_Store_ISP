...
function selectedRowToInput() {
    ...
    for (var i = 1; i < table.rows.length; i++) {
        table.rows[i].onclick = function () {
            ...
            document.getElementById("supplier_id").value = this.cells[2].textContent;
...
function save() {
    ...
    var action = "insert";
    if (grn_id === "" || supplier_id === "" || total === "" || status === "" || date_time === "") {
        alert("Please Enter All Details")
    } else {
        $.ajax({
            url: 'GrnServlet',
            method: 'POST',
            data: {action: action, grn_id: grn_id, supplier_id: supplier_id, total: total, status: status, date_time: date_time},
...
function update() {
    ...
    } else if (grn_id === "" || supplier_id === "" || total === "" || status === "" || date_time === "") {
        alert("Please Enter All Details")
    } else {
        $.ajax({
            url: 'GrnServlet',
...
function load() {
    ...
    var dropdown1 = document.getElementById('supplier_id');
    $('#supplier_id')
            .find('option')
            .remove()
            .end()
